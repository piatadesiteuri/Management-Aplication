-- Adaugă coloane pentru integrarea cu evenimente
ALTER TABLE tasks ADD COLUMN event_id INT NULL;
ALTER TABLE tasks ADD COLUMN task_type ENUM('MANUAL', 'AUTO_GENERATED', 'EVENT_RELATED') DEFAULT 'MANUAL';
ALTER TABLE tasks ADD COLUMN workflow_step VARCHAR(50) NULL;
...
CREATE INDEX idx_tasks_workflow_step ON tasks(workflow_step);
CREATE INDEX idx_tasks_parent_task_id ON tasks(parent_task_id);

-- Adaugă foreign key pentru event_id
ALTER TABLE tasks ADD CONSTRAINT fk_tasks_event_id 
FOREIGN KEY (event_id) REFERENCES calendar_events(id) ON DELETE SET NULL;

-- Adaugă foreign key pentru parent_task_id (task-uri dependente)
